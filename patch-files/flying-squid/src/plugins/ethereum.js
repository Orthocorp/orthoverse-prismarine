...
module.exports.player = function (player, serv) {
  ...
  player._client.on('ethereum', (msg) => {
    ...
    if (msg.slice(0, 5) === 'save:') {
      ...
      // file name format: <slot>-<realm>.lnd Note that the region files containing the land data are in *.mca files in flying-squid/world/region/
      ...
      const saveFileName = slot.toString() + '-' + voxel[landPos][2] + '.lnd'

      // mca stands for minecraft anvil region
      // a chunk is a 16x256x16 column of data. An Orthoverse land is a 96 block wide square, making it a 6 by 6 collection of chunks
      // so to save a land we just need to save an array of 6x6 = 36 chunks

      for (let chunkZ = lat * 16; chunkZ < (lat * 16) + 6; chunkZ++) { 
        for (let chunkX = long * 16; chunkX < (long * 16) + 6; chunkX++) { 
          ...
          try {
            ...
            console.log(chunkDump)
            console.log(chunkDump.length)
            //const chunk2 = new Chunk()
            //chunk2.load(chunk.dump())
          } catch (e) {
            player._client.writeChannel('ethereum', 'mesg:Error chunk: ' + e)
          }

          try {
            fs.writeFileSync(savePath + chunkX.toString() + '.' + chunkZ.toString() + '.' + saveFileName, chunkDump);
          } catch (e) {
    ...
    if (msg.slice(0, 5) === 'load:') {
      ...
      // check if land save folder exists and make it if it doesn't
      const loadFileName = slot.toString() + '-' + voxel[landPos][2] + '.lnd'

      for (let chunkZ = lat * 16; chunkZ < (lat * 16) + 6; chunkZ++) { 
        for (let chunkX = long * 16; chunkX < (long * 16) + 6; chunkX++) { 
          try {
            const chunkData = new Buffer.from(fs.readFileSync(loadPath + chunkX.toString() + '.' + chunkZ.toString() + '.' + loadFileName));
