...
class World extends EventEmitter {
  constructor (chunkGenerator, storageProvider = null, savingInterval = 2000) {
    super()
  ...
  getLoadedColumn (chunkX, chunkZ) {
    ...
    if (this.columns[key] === null) {console.log("There is a null column")}
  ...
  async getColumn (chunkX, chunkZ) {
    ...
    if (!this.columns[key]) {
      ...
      if (this.storageProvider != null) {
        ...
        // force chunk from generator if (data != null) { chunk = data; console.log("Getting chunk from file") }
      }
      const loaded = chunk != null
      if (!loaded && this.chunkGenerator) {
        chunk = await this.chunkGenerator(chunkX, chunkZ)
        console.log("Getting chunk from generator")
      }
  ...
  async saveNow () {
    ...
    for (const [key, { chunkX, chunkZ }] of this.savingQueue.entries()) {
      ...
      } else { console.log("Trying to save undefined column") }
    }
  ...
  queueSaving (chunkX, chunkZ) {
    if (typeof this.columns[columnKeyXZ(chunkX, chunkZ)] === 'undefined') {
      console.log("Queued null column for saving")
    }
  ...
  async getColumnAt (pos) {
    const chunkX = Math.floor(pos.x / 16)
    const chunkZ = Math.floor(pos.z / 16)
    const currentColumn = await this.getColumn(chunkX, chunkZ)
