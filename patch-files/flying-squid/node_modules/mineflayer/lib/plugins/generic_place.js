...
function inject (bot) {
  ...
  async function _genericPlace (referenceBlock, faceVector, options) {
    ...
    if (options.offhand) {
      ...
      handToPlaceWith = 1
    } else if (!bot.heldItem && !options.ctrlKey) {
      throw new Error('must be holding an item to place')
    } else if (!bot.heldItem && options.ctrlKey) {
      // place water
      throw new Error('Will be doing water place with this')
    }
    ...
    console.log('Held item is ', bot.heldItem.name)
    if (options.ctrlKey && bot.heldItem.name === 'dirt') {
      console.log("Control with dirt to make grass")
      bot.heldItem.name = 'grass'
      bot.heldItem.type = 8
