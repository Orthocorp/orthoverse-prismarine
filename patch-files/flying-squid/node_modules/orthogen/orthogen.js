...
function generation ( { version = '1.15.2', seed } = {}) {
  ...
  function generateSimpleChunk (serv, chunkX, chunkZ) {
    ...
    if !(landKey in serv.voxel.data) {
      ...
      if !("slot" in serv.voxel.data[landKey]) {
        serv.voxel.data[landKey][slot] = 0 // indicates use save slot 0
      ...
      return serv.voxel.loadChunkFromFile(serv.voxel.data[landKey][slot],chunkX, chunkZ)
    }
    */
  }
  ...
  function generateDefaultLandChunks (serv, landX, landZ) {
    ...
    const landKey = serv.voxel.data[landX.toString() + ':' + landZ.toString()]
    const chunkX = landX*6
    const chunkZ = landZ*6
    for (let x = chunkX; x < chunkX + 6; x++) {
      for (let y = chunkX; y < chunkX + 6; y++) {
        // now we save the default land without edits in slot 0
        this.voxel.saveChunkToFile(0, generateBasicChunk(serv, x, y))
      }
    }
  }
