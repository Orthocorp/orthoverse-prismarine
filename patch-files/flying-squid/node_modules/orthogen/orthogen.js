...
function lush(chunk, height, seedRand) {
  for (let x = 0; x < 16; x++) {
    for (let y = 0; y < 16; y++) {
      const pos2 = new Vec3(x, height + 1, y)
      if (seedRand(18) === 0) {
        chunk.setBlockType(pos2, mcData.blocksByName.tall_grass.id)
        chunk.setSkyLight(pos2, 15)
      }
    }
  }
...
function generation ({ version = '1.16.1', seed } = {}) {
  ...
  function generateSimpleChunk (chunkX, chunkY) {
    ...
    const thisRand = seedRand(17)
    if (thisRand === 0) { hill(chunk, level, seedRand, true) }; // lush hill
    if (thisRand === 1) { mountain(chunk, level, seedRand, true) }; // lush mountain
    if (thisRand === 2) { lake(chunk, seedRand) };
    if (thisRand === 3) { }; // plains - default terrain
    if (thisRand === 4) { deciduous(chunk, level, seedRand) };
    if (thisRand === 5) { lush(chunk, level, seedRand) }; // lush plains
    if (thisRand === 6) { quarry(chunk, seedRand, true) };
    if (thisRand === 7) { evergreen(chunk, level, seedRand) };
    if (thisRand === 8) { hill(chunk, level, seedRand, false) }; // barren hill
    if (thisRand === 9) { mountain(chunk, level, seedRand, false) }; // barren mountain
    if (thisRand === 10) { lake(chunk, seedRand) };
    if (thisRand === 11) { desert(chunk, seedRand, level, true) }; // desert plains
    if (thisRand === 12) { deciduous(chunk, level, seedRand) };
    if (thisRand === 13) { desert(chunk, seedRand, level, false) }; // badlands
    if (thisRand === 14) { quarry(chunk, seedRand, false) }; // cobblestone quarry
    if (thisRand === 15) { deciduous(chunk, level, seedRand) };
    if (thisRand === 16) { sea(chunk, seedRand) };
    return chunk
  }
