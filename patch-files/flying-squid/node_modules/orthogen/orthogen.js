...
function lake(chunk, seedRand) {
  ...
  for (let x = 0; x < 16; x++) {
    for (let y = 0; y < 16; y++) {
      for (let h = 14; h < 15 + seedRand(3); h++) {
        const pos = new Vec3(x, h, y)
...
function generation ({ version = '1.16.1', seed } = {}) {
  ...
  function generateSimpleChunk (chunkX, chunkY) {
    ...
    const thisRand = seedRand(10)
    if (thisRand === 1) { mountain(chunk, level, seedRand) };
    if (thisRand === 2) { lake(chunk, seedRand) };
    if (thisRand === 4) { deciduous(chunk, level, seedRand) };
    if (thisRand === 5) { evergreen(chunk, level, seedRand) };
    if (thisRand === 6) { evergreen(chunk, level, seedRand) };
    if (thisRand === 8) { hill(chunk, level, seedRand) };
    return chunk
  }
