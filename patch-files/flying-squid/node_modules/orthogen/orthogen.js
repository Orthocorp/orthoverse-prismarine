...
function generation ( { version = '1.15.2', seed } = {}) {
  ...
  // Takes a server, and an Orthoverse X and Y position, and generates the 36 chunks that
  // it consists of, for saving as a default land save
  function generateDefaultLandChunks (serv, tileX, tileY) {
    const chunkX = tileX*6
    const chunkY = tileY*6
    const landChunks = []
    for (let x = chunkX; x < chunkX + 6; x++) {
      for (let y = chunkX; y < chunkX + 6; y++) {
        landChunks.push(generateBasicChunk(serv, x, y))
      }
    }
    return landChunks
  }

  function generateSimpleChunk (serv, chunkX, chunkY) {
    return generateBasicChunk(serv, chunkX, chunkY)
  }

  function generateBasicChunk (serv, chunkX, chunkY) {
    ...
    */
    ...
    if (tileKey in serv.voxel.data) {
      tokenId = serv.voxel.data[tileKey][0];
    }
    ...
    } else if (tileKey in serv.voxel.data) {
      castleLvl = serv.voxel.data[tileKey][2] % 8;
      realm = (serv.voxel.data[tileKey][2] > 7) ? 1 : 0;
      // standard tile, but may contain a castle!
      feature = serv.voxel.data[tileKey][0][((posY * 6) + posX) + 2];
      // castle
