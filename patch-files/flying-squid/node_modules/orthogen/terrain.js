    ...
    for (let x = 0; x < 16; x++) {
      for (let y = 0; y < 16; y++) {
        ...
        if ( h === 5) {
          ...
          chunk.setBlockType(pos, mcData.blocksByName.water.id)
          if ((h == 8 + seedRand(6)) && (seedRand(2) == 1)) {
            chunk.setBlockType(pos, mcData.blocksByName.seagrass.id)
          }
        } else { 
          chunk.setBlockType(pos, mcData.blocksByName.air.id)
    ...
    for (let y = 0; y < 16; y++) {
      ...
      for (let z = 0; z < level + 1; z++) {
        ...
        if (z < bedrockheight) { 
          ...
          block = mcData.blocksByName.black_concrete.id  // Set surface
        }
        const pos = new Vec3(x, z, y)
        if (block) { 
          chunk.setBlockType(pos, block)
...
function square(chunk, level, lush) {
  const crete = mcData.blocksByName.gray_concrete.id;
  for (let x = 1; x < 15; x++) {
    const pos = new Vec3(x, level, 1)
    chunk.setBlockType(pos, crete)
    chunk.setBlockType(pos.offset(0,0,13), crete)
    const posB = new Vec3(1, level, x)
    chunk.setBlockType(posB, crete)
    chunk.setBlockType(posB.offset(13,0,0), crete)
  }
  let block
  let data
  if (lush === true) {
    block = mcData.blocksByName.gray_concrete.id; 
    data = 11
  } else {
    block = mcData.blocksByName.gray_concrete.id;
    data = 5 
  ...
  chunk.setBlockType(pos2, block)
  chunk.setSkyLight(pos2, 15)
  chunk.setBlockData(pos2, data)
  chunk.setBlockType(pos2.offset(0,0,1), block)
  chunk.setBlockData(pos2.offset(0,0,1), data)
  chunk.setBlockType(pos2.offset(1,0,1), block)
  chunk.setBlockData(pos2.offset(1,0,1), data)
  chunk.setBlockType(pos2.offset(1,0,0), block)
  chunk.setBlockData(pos2.offset(1,0,0), data)
}

function cube(chunk, level, lush) {
  const block = mcData.blocksByName.black_concrete.id;
  ...
  const pos = new Vec3(13,level + 1,7)
  chunk.setBlockType(pos, air)
  chunk.setSkyLight(pos, 15)
...
function prism(chunk, level, lush) {
  ...
  for (let h = 0; h < 7; h++) {
    for (let x = 3; x < 13; x++) {
      for (let z = 5 + h; z < 11; z++) {
        const pos = new Vec3(x, level + h + 1, z)
        chunk.setBlockType(pos, mcData.blocksByName.black_concrete.id)
        chunk.setSkyLight(pos, 15) 
      } 
    }
  }
  ...
  const pos = new Vec3(12,level + 1,8)
  chunk.setBlockType(pos, air)
  chunk.setSkyLight(pos, 15)
  chunk.setBlockType(pos.offset(0,0,1), air)
  chunk.setSkyLight(pos.offset(0,0,1), 15)
...
function pyramid(chunk, level, lush) {
  ...
  const pos = new Vec3(7, level + 1, 0)
  chunk.setBlockType(pos.offset(0,0,0), mcData.blocksByName.air.id)
  chunk.setSkyLight(pos.offset(0,0,0), 15)
  chunk.setBlockType(pos.offset(1,0,0), mcData.blocksByName.air.id)
  chunk.setSkyLight(pos.offset(1,0,0), 15)

  chunk.setBlockType(pos.offset(0,1,1), mcData.blocksByName.air.id)
  chunk.setSkyLight(pos.offset(0,1,1), 15)
  chunk.setBlockType(pos.offset(1,1,1), mcData.blocksByName.air.id)
  chunk.setSkyLight(pos.offset(1,1,1), 15)

  chunk.setBlockType(pos.offset(-1,1,0), mcData.blocksByName.black_concrete.id)
  chunk.setSkyLight(pos.offset(-1,1,0), 15)
  chunk.setBlockType(pos.offset(2,1,0), mcData.blocksByName.black_concrete.id)
  chunk.setSkyLight(pos.offset(2,1,0), 15)
  chunk.setBlockType(pos.offset(-1,2,0), mcData.blocksByName.black_concrete.id)
  chunk.setSkyLight(pos.offset(-1,2,0), 15)
  chunk.setBlockType(pos.offset(2,2,0), mcData.blocksByName.black_concrete.id)
  chunk.setSkyLight(pos.offset(2,2,0), 15)
  chunk.setBlockType(pos.offset(0,3,0), mcData.blocksByName.black_concrete.id)
  chunk.setSkyLight(pos.offset(0,3,0), 15)
  chunk.setBlockType(pos.offset(1,3,0), mcData.blocksByName.black_concrete.id)
  chunk.setSkyLight(pos.offset(1,3,0), 15)


  chunk.setBlockType(pos.offset(-1,2,1), mcData.blocksByName.black_concrete.id)
  chunk.setSkyLight(pos.offset(-1,2,1), 15)
  chunk.setBlockType(pos.offset(2,2,1), mcData.blocksByName.black_concrete.id)
  chunk.setSkyLight(pos.offset(2,2,1), 15)
  chunk.setBlockType(pos.offset(0,2,1), mcData.blocksByName.black_concrete.id)
...
function pit(chunk, lush) {
  let block;
  if (lush) { block = mcData.blocksByName.grass_block.id } else { block = mcData.blocksByName.dirt.id }
  const liner = mcData.blocksByName.black_concrete.id
  for (let h = 14; h < 24; h++) {
    for (let x = 0; x < 16; x++) {
      for (let y = 0; y < 16; y++) {
        let data = 1;
...
function dome(chunk, level, lush) {
  const pos = new Vec3(0, level, 0)
  for (let h = 1; h < 4; h++) {
    for (let l = 0; l < 4; l++) {
      chunk.setBlockType(pos.offset(6+l,h,2), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(6+l,h,2), 15)
      chunk.setBlockType(pos.offset(6+l,h,13), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(6+l,h,13), 15) 
      chunk.setBlockType(pos.offset(2,h,6+l), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(2,h,6+l), 15)
      chunk.setBlockType(pos.offset(13,h,6+l), mcData.blocksByName.black_concrete.id)
  ...
  for (let h = 1; h < 4; h++) {
    for (let l = 0; l < 2; l++) {
      ...
      chunk.setBlockType(pos.offset(4+l,h,3), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(4+l,h,3), 15)
      chunk.setBlockType(pos.offset(10+l,h,3), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(10+l,h,3), 15) 
      chunk.setBlockType(pos.offset(3,h,4+l), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(3,h,4+l), 15)
      chunk.setBlockType(pos.offset(12,h,4+l), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(12,h,4+l), 15)  

      chunk.setBlockType(pos.offset(4+l,h,12), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(4+l,h,12), 15)
      chunk.setBlockType(pos.offset(10+l,h,12), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(10+l,h,12), 15) 
      chunk.setBlockType(pos.offset(3,h,10+l), mcData.blocksByName.black_concrete.id)
      chunk.setSkyLight(pos.offset(3,h,10+l), 15)
      chunk.setBlockType(pos.offset(12,h,10+l), mcData.blocksByName.black_concrete.id)
  ...
  for (let i = 0; i < seals1.length; i++) {
    chunk.setBlockType(pos.offset(seals1[i][0],4,seals1[i][1]), mcData.blocksByName.black_concrete.id)
  ...
  for (let i = 0; i < seals2.length; i++) {
    chunk.setBlockType(pos.offset(seals2[i][0],6,seals2[i][1]), mcData.blocksByName.black_concrete.id)
  ...
  for (let h = 4; h < 6; h++) {
    for (let l = 0; l < 2; l++) {
      for (let i = 0; i < 6; i++) {
        chunk.setBlockType(pos.offset(xstarts[i][0] + l,h,xstarts[i][1]), mcData.blocksByName.black_concrete.id)
        chunk.setSkyLight(pos.offset(xstarts[i][0] + l,h,xstarts[i][1]), 15)
        chunk.setBlockType(pos.offset(zstarts[i][0],h,zstarts[i][1] + l), mcData.blocksByName.black_concrete.id)
        chunk.setSkyLight(pos.offset(zstarts[i][0],h,zstarts[i][1] + l), 15)
      }
    }
  }
  for (let l = 0; l < 4; l++) {
    chunk.setBlockType(pos.offset(6+l,6,5), mcData.blocksByName.black_concrete.id)
    chunk.setSkyLight(pos.offset(6+l,6,5), 15)
    chunk.setBlockType(pos.offset(6+l,6,10), mcData.blocksByName.black_concrete.id)
    chunk.setSkyLight(pos.offset(6+l,6,10), 15) 
    chunk.setBlockType(pos.offset(5,6,6+l), mcData.blocksByName.black_concrete.id)
    chunk.setSkyLight(pos.offset(5,6,6+l), 15)
    chunk.setBlockType(pos.offset(10,6,6+l), mcData.blocksByName.black_concrete.id)
    chunk.setSkyLight(pos.offset(10,6,6+l), 15) 
    for (let m = 0; m < 4; m++) {
      chunk.setBlockType(pos.offset(6+l,7,6 + m), mcData.blocksByName.black_concrete.id)
