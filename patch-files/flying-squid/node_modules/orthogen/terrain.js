  ...
  for (let x = 0; x < 16; x++) {
    for (let y = 0; y < 16; y++) {
      ...
      for (let z = 0; z < 256; z++) {
        let block
        let data
        // set a bedrock bottom at first layer and at 32
        if ((z < bedrockheight) || (z === 32)) { 
          block = mcData.blocksByName.bedrock.id
        // 32 layers of dirt and rock
        } else if (z < 32) {
          block = (seedRand(10) < 9) ? mcData.blocksByName.dirt.id : mcData.blocksByName.granite.id
        } else if (z <64) {
          block = mcData.blocksByName[fantasyBlocks[Math.floor(Math.random() * fantasyBlocks.length)]].id
