...
const voxel = require('../../../../lands/voxel.json');
...
function inject (bot) {
  ...
  // update to canDigBlock to only enable if in own land
  function canDigBlock (block) {
    if (block && block.diggable && block.position.offset(0.5, 0.5, 0.5).distanceTo(bot.entity.position.offset(0, 1.65, 0)) <= 5.1) {
      // work out if digging in owned area
      const landPos = Math.floor(block.position.x / 96).toString() + ":" + Math.floor(block.position.z / 96).toString()
      if (landPos in voxel) {
        const blockOwned = voxel[landPos][4]
        if (bot.entity.ethereum.wallet === blockOwned) {
  ...
  function digTime (block) {
    ...
    /* const diggingTime = block.digTime(
      type,
      creative,
    ...
    console.log("Digging time: ", diggingTime) */
    const diggingTime = 600 // should be fast enough without being instant
    return diggingTime
  }
