...
class Viewer {
  ...
  setFirstPersonCamera (pos, yaw, pitch, bot) {
    if (pos) {
      new TWEEN.Tween(this.camera.position).to({ x: pos.x, y: pos.y + 1.6, z: pos.z}, 50).start()
    }
  ...
  setThirdPersonCamera (pos, yaw, pitch, bot) {
    if (pos) {
      ...
      const pitchMod = (pitch + Math.PI/2) % (2 * Math.PI)
      const zadj = -1 * Math.cos(pitchMod) * 3
      const xadj = Math.sin(pitchMod) * 3
    ...
    this.camera.rotation.set(-0.20*Math.PI, yaw + Math.PI, 0, 'ZYX')
    this.entities.update(bot)
  }
