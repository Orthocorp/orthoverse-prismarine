...
class Cursor {
  ...
  update (bot) {
    ...
    if (cursorBlock && this.buttons[2] && (!this.lastButtons[2] || cursorChanged) && this.lastPlaced >= 4) {
      ...
      const options = {delta, forceLook: 'ignore'}
      options['shiftKey'] = this.shiftKey
      options['ctrlKey'] = this.ctrlKey
      bot._placeBlockWithOptions(cursorBlock, vecArray[cursorBlock.face], options)
      bot.lastPlaced = 0
    }
